...
function dice (diceData: string) {
  ...
  const diceCommand = from(String(diceData.match(/^[1-9][0-9]*[dD][1-9][0-9]*/)).split('D')).select(item => Number(item)).toArray()
  const results = []
  ...
  if (thresholdData) {
    ...
    if (thresholdData[3] !== undefined) {
      ...
      if (data1 && data2) ans += (((dicen === '1d100') && (total <= 5)) ? '決定的成功' : '成功')
